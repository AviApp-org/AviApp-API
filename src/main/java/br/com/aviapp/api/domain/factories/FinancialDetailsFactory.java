...
public class FinancialDetailsFactory extends CollectCalculator {
    ...
    public static FinancialDetailsVO createFinancialDetails(String aviaryName, EggValueBO eggValueBO, List<CollectEggBO> eggCollects) {
        ...
        BigDecimal marketEggsValue = BigDecimal.valueOf(marketEggs).multiply(BigDecimal.valueOf(eggValueBO.getValue())).stripTrailingZeros();

        BigDecimal hatchableEggsValue = BigDecimal.valueOf(hatchableEggs).multiply(BigDecimal.valueOf(eggValueBO.getValue())).stripTrailingZeros();

        BigDecimal totalValue = marketEggsValue.add(hatchableEggsValue).stripTrailingZeros();

        return new FinancialDetailsVO(aviaryName, hatchableEggsValue, marketEggsValue, totalValue);
    }
}
